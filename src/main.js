...
const collections = vocabularyData.collections || [];
...
const state = {
  ...
  selectedChapters: new Set([1]), selectedTopics: new Set(), selectedCollections: new Set(), learningSettings: {...DEFAULT_LEARN_SETTINGS}, statsSource: "all", searchSource: "all", searchTopic: "all",
...
function updateDocumentMeta() {
  ...
  const description = landing ? "Türkisch lernen für Anfänger: kostenloser Türkisch-Vokabeltrainer mit 45 Buchkapiteln, Themenwortschatz, 2.000 gebräuchlichen Vokabeln und Lehrbuch." : "Kostenloser Türkisch-Vokabeltrainer für Anfänger: 2.969 Vokabeln aus 45 Kapiteln, 11 Themen und einer Häufigkeitsrubrik mit 2.000 Wörtern.";
      ...
      <section class="landing-hero" aria-labelledby="landing-title">
        <div class="landing-copy">
          ...
          <ul class="landing-benefits" aria-label="Vorteile des Vokabeltrainers">
            ...
            <li><span aria-hidden="true">✓</span> Die 2.000 gebräuchlichsten Vokabeln</li>
...
function chaptersView() {
  ...
  return `<section class="content-head"><p class="eyebrow">KAPITEL, THEMEN & RUBRIKEN</p><h2>Wortschatz auswählen</h2><p>${state.selectedChapters.size} Kapitel · ${state.selectedTopics.size} Themen · ${state.selectedCollections.size} Rubriken ausgewählt</p></section>
  ...
  <section><div class="section-title topic-heading"><h3 class="group-heading">Themenwortschatz</h3><div class="tool-buttons"><button data-topic-select="all">Alle</button><button data-topic-select="invert">Umkehren</button><button data-topic-select="none">Zurücksetzen</button></div></div><div class="chapter-grid topic-grid">${topics.map(t=>selectionCard(topicStats(t.id), "topic")).join("")}</div></section>
  <section class="collection-section"><h3 class="group-heading">Weitere Rubriken</h3><div class="chapter-grid collection-grid">${collections.map(collectionCard).join("")}</div></section></div>
  <button class="primary floating-start" data-action="selected-start" ${state.selectedChapters.size||state.selectedTopics.size||state.selectedCollections.size?"":"disabled"}>Auswahl lernen (${state.selectedChapters.size+state.selectedTopics.size+state.selectedCollections.size})</button>`;
}
...
function progressView() {
  ...
  <div class="source-tabs panel" role="group" aria-label="Statistikquelle">${[["all","Gesamtes Vokabular"],["chapter","Kapitelvokabular"],["topic","Themenwortschatz"],["frequency","2.000 gebräuchlichste"]].map(([v,l])=>`<button data-stats-source="${v}" class="${state.statsSource===v?"active":""}">${l}</button>`).join("")}</div>
  ...
  <article class="panel"><h3>Fortschritt je Rubrik</h3><div class="chapter-progress-list">${["all","chapter"].includes(state.statsSource)?Array.from({length:45},(_,i)=>chapterStats(i+1)).map(c=>`<div><span>${c.chapter}</span>${progressBar(c.mastery,100)}<small>${c.mastery}%</small></div>`).join(""):""}${["all","topic"].includes(state.statsSource)?topics.map(t=>{const c=topicStats(t.id);return `<div class="topic-progress"><span>${esc(c.title)}</span>${progressBar(c.mastery,100)}<small>${c.mastery}%</small></div>`}).join(""):""}${["all","frequency"].includes(state.statsSource)?collections.map(def=>{const c=collectionStats(def.id);return `<div class="topic-progress"><span>${esc(c.title)}</span>${progressBar(c.mastery,100)}<small>${c.mastery}%</small></div>`}).join(""):""}</div></article></section>`;
}
...
function searchVocabulary(q) { const n=searchKey(q); return vocab.filter(v=>(state.searchSource==="all"||v.sourceType===state.searchSource)&&(state.searchTopic==="all"||v.sourceType!=="topic"||v.topicId===state.searchTopic)&&searchKey(`${v.turkish} ${v.german.join(" ")} ${v.chapter||""} ${v.chapterTitle||""} ${v.topicTitle||""} ${v.collectionTitle||""} ${v.frequencyRank||""} ${v.category}`).includes(n)).sort((a,b)=>a.german[0].localeCompare(b.german[0],"de")); }
...
function bindChapterActions() {
  ...
  document.querySelectorAll("[data-collection]").forEach(c=>c.addEventListener("change",()=>{c.checked?state.selectedCollections.add(c.dataset.collection):state.selectedCollections.delete(c.dataset.collection);saveVocabularySelection();render();}));
  ...
  document.querySelectorAll("[data-start-collection]").forEach(b=>b.addEventListener("click",()=>{state.selectedCollections=new Set([b.dataset.startCollection]);saveVocabularySelection();state.learningSettings={...state.learningSettings,scope:"collections"};saveMeta("learningSettings",state.learningSettings);navigate("learn");}));
...
async function recordDaily(quality){let day=state.daily.find(d=>d.date===today());if(!day){day={date:today(),answered:0,correct:0,wrong:0,ids:[]};state.daily.push(day);}day.answered++;if(["correct","easy"].includes(quality))day.correct++;if(quality==="wrong")day.wrong++;const v=state.session.items[state.session.index];if(!day.ids.includes(v.id))day.ids.push(v.id);await db.put("daily",day);if(v.chapter)await saveMeta("lastChapter",v.chapter);if(v.topicId)await saveMeta("lastTopic",v.topicId);if(v.collectionId)await saveMeta("lastCollection",v.collectionId);await saveMeta("lastMode",state.session.mode);await saveMeta("selectedChapters",[...state.selectedChapters]);await saveMeta("selectedTopics",[...state.selectedTopics]);await saveMeta("selectedCollections",[...state.selectedCollections]);}
...
function saveVocabularySelection(){return Promise.all([saveMeta("selectedChapters",[...state.selectedChapters]),saveMeta("selectedTopics",[...state.selectedTopics]),saveMeta("selectedCollections",[...state.selectedCollections])]);}
...
async function resetData(){if(!confirm("Wirklich den vollständigen Lernstand löschen? Diese Aktion kann nicht rückgängig gemacht werden."))return;if(prompt('Zur Bestätigung bitte "LÖSCHEN" eingeben:')!=="LÖSCHEN")return toast("Zurücksetzen abgebrochen.",true);await Promise.all(["progress","meta","daily"].map(s=>db.clear(s)));state.progress.clear();state.meta.clear();state.daily=[];state.selectedChapters=new Set([1]);state.selectedTopics.clear();state.selectedCollections.clear();state.learningSettings={...DEFAULT_LEARN_SETTINGS};render();toast("Der Lernstand wurde vollständig zurückgesetzt.");}
